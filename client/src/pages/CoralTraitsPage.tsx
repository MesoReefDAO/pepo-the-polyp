      ...
      <main className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex items-start justify-between gap-3 flex-wrap">
            <div>
              ...
              <p className="text-xs md:text-sm text-[#d4e9f380] max-w-2xl leading-relaxed">
                {t("coralTraits.introPre")} <a href="https://coraltraits.org" target="_blank" rel="noopener noreferrer" className="text-[#83eef0] no-underline hover:underline">coraltraits.org</a>{" "}
                {t("coralTraits.introMid", {
                  species: (summary?.species ?? 0).toLocaleString(),
                  measurements: (summary?.measurements ?? 0).toLocaleString(),
                })}{" "}
                <Link href="/reef-map" data-testid="link-reef-map-inline" className="text-[#83eef0] no-underline hover:underline">{t("coralTraits.reefMapLink")}</Link>.
              </p>
            </div>
            <div className="flex gap-2 flex-wrap items-center">
              <Stat label={t("coralTraits.statSpecies")}      value={summary?.species ?? "..."}      testId="stat-species" color="#83eef0" />
              <Stat label={t("coralTraits.statMeasurements")} value={summary?.measurements ?? "..."} testId="stat-measurements" color="#f9ca24" />
              <Stat label={t("coralTraits.statTraits")}       value={summary?.traits ?? "..."}       testId="stat-traits" color="#a6ce39" />
              <Stat label={t("coralTraits.statLocations")}    value={summary?.locations ?? "..."}    testId="stat-locations" color="#26de81" />
            </div>
          </div>
        </header>
        ...
        <div className="px-4 md:px-8 py-3 flex items-center gap-2 border-b border-[#ffffff08] flex-wrap">
          <input
            ...
            placeholder={t("coralTraits.searchPlaceholder")}
          ...
          >
            <option value="">{t("coralTraits.allFamilies")}</option>
          ...
          >
            {openMobile ? t("coralTraits.showList") : t("coralTraits.showDetail")}
          </button>
        </div>
        ...
        <div className="flex-1 grid grid-cols-1 md:grid-cols-[280px_1fr_320px] overflow-hidden">
          ...
          >
            {speciesLoading && (
              <div className="p-4 text-xs text-[#d4e9f380]">{t("coralTraits.loadingSpecies")}</div>
            )}
            {!speciesLoading && filteredSpecies.length === 0 && (
              <div className="p-4 text-xs text-[#d4e9f380]">{t("coralTraits.noSpeciesMatch")}</div>
            )}
            <ul className="py-1">
            ...
            {hasNextPage && (
              <div className="px-4 py-3">
                ...
                >
                  {isFetchingNextPage ? t("coralTraits.loadingShort") : t("coralTraits.loadMoreSpecies", { loaded: filteredSpecies.length, total: (summary?.species ?? 0).toLocaleString() })}
                </button>
              </div>
            )}
          </aside>
          ...
          >
            {!selected && (
              <div className="text-xs text-[#d4e9f380]">{t("coralTraits.selectSpeciesPrompt")}</div>
            )}
            {selected && (
              <>
                ...
                <div className="flex flex-wrap gap-2 mt-2 mb-3 text-[10px]">
                  ...
                  {selected.familyMolecules && <Tag label={t("coralTraits.family", { name: selected.familyMolecules })} color="#83eef0" />}
                  {selected.familyMorphology && selected.familyMorphology !== selected.familyMolecules && (
                    <Tag label={t("coralTraits.morphology", { name: selected.familyMorphology })} color="#26de81" />
                  )}
                  {selected.synonymSpecies && (
                    ...
                    >
                      {t("coralTraits.syn")} <span className="italic">{selected.synonymSpecies}</span>
                    </span>
                  )}
                  <a
                ...
                <div className="flex items-center gap-2 mt-4 mb-2 flex-wrap">
                  <h3 className="text-xs uppercase tracking-widest text-[#d4e9f366]">
                    {t("coralTraits.measurements")} ({visibleMeasurements.length}{traitFilter ? ` ${t("coralTraits.measurementsOf", { total: measurements?.length ?? 0 })}` : ""})
                  </h3>
                  {measTraitNames.length > 0 && (
                    ...
                    >
                      <option value="">{t("coralTraits.allTraits", { count: measTraitNames.length })}</option>
                ...
                {measLoading ? (
                  <div className="text-xs text-[#d4e9f380] py-4">{t("coralTraits.loadingMeasurements")}</div>
                ) : visibleMeasurements.length === 0 ? (
                  <div className="text-xs text-[#d4e9f380] py-4">{t("coralTraits.noMeasurements")}</div>
                ) : (
                  <div className="overflow-x-auto rounded-lg border border-[#ffffff08]">
                    <table className="w-full text-xs" data-testid="table-coral-samples">
                      <thead>
                        <tr className="bg-[#0a293366] text-[#d4e9f380] text-left uppercase tracking-widest text-[9px]">
                          <th className="px-2 py-2">{t("coralTraits.thTrait")}</th>
                          <th className="px-2 py-2">{t("coralTraits.thValue")}</th>
                          <th className="px-2 py-2">{t("coralTraits.thUnit")}</th>
                          <th className="px-2 py-2">{t("coralTraits.thLocation")}</th>
                          <th className="px-2 py-2">{t("coralTraits.thMethod")}</th>
                        </tr>
                      </thead>
                      <tbody>
                    ...
                    {visibleMeasurements.length > 300 && (
                      <div className="px-2 py-2 text-[10px] text-[#d4e9f366] text-center">
                        {t("coralTraits.showing300", { count: visibleMeasurements.length })}
                      </div>
                    )}
                  </div>
                )}
          ...
          <aside data-testid="pane-coral-trait-reference" className="hidden md:block border-l border-[#ffffff08] overflow-y-auto p-4">
            <h3 className="text-xs uppercase tracking-widest text-[#d4e9f366] mb-2">{t("coralTraits.traitReference")}</h3>
            <p className="text-[10px] text-[#d4e9f366] mb-3">
              {t("coralTraits.traitsGrouped", { count: traits?.length ?? 0 })}
            </p>
                ...
                .map(([cls, list]) => (
                  <details key={cls} className="text-xs">
                    <summary className="cursor-pointer text-[#d4e9f3] font-semibold py-1 hover:text-[#83eef0]" data-testid={`summary-trait-class-${cls}`}>
                      {t("coralTraits.class", { cls })} <span className="text-[#d4e9f366] font-normal">({list.length})</span>
                    </summary>
