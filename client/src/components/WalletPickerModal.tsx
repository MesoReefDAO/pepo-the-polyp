    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      >
        ...
        <div className="overflow-y-auto flex-1 px-4 pb-6">
          ...
          {/* ── WalletConnect (Ledger / Trezor / 300+) ── */}
          <div className="mb-5">
            <SectionDivider label="More wallets" />
            <WcRow busy={busy} onClick={openPrivyFallback} />
          </div>
...
function WcRow({ busy, onClick }: { busy: boolean; onClick: () => void }) {
  return (
    <button
      onClick={busy ? undefined : onClick}
      disabled={busy}
      data-testid="wallet-option-walletconnect"
      className="mt-2.5 flex items-center gap-3 w-full px-4 py-3 rounded-2xl border border-[#3B99FC1a] bg-[#3B99FC06] hover:bg-[#3B99FC0e] transition-colors text-left disabled:opacity-50"
    >
