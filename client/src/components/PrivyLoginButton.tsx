...
function LinkedInDot() {
  return (
    <svg width="13" height="13" viewBox="0 0 24 24" fill="#0A66C2" className="flex-shrink-0">
...
function WalletDot() {
  return (
    <svg width="13" height="13" viewBox="0 0 24 24" fill="#83eef0" className="flex-shrink-0">
...
function ProviderIcon({ user }: { user: any }) {
  ...
  const hasGoogle = linked.some((a: any) => a.type === "google_oauth");
  const hasTwitter = linked.some((a: any) => a.type === "twitter_oauth");
  const hasLinkedIn = linked.some((a: any) => a.type === "linkedin_oauth");
  const hasWallet = linked.some((a: any) => a.type === "wallet");

  if (hasGoogle) return <GoogleDot />;
  if (hasTwitter) return <XDot />;
  if (hasLinkedIn) return <LinkedInDot />;
  if (hasWallet) return <WalletDot />;
  return null;
}
...
export function PrivyLoginButton() {
  ...
  return (
    <Button
      ...
      className="relative inline-flex items-center justify-center px-5 md:px-6 py-2 h-auto rounded-full bg-[linear-gradient(170deg,rgba(131,238,240,1)_0%,rgba(63,176,179,1)_100%)] border-none shadow-none hover:opacity-90 transition-opacity"
    >
      <span className="absolute inset-0 bg-[#ffffff01] rounded-full shadow-[0px_4px_6px_-4px_#83eef033,0px_10px_15px_-3px_#83eef033]" />
      <span className="relative [font-family:'Inter',Helvetica] font-normal text-[#00585a] text-sm md:text-base text-center leading-6 whitespace-nowrap">
        Log in
      </span>
    </Button>
  );
