...
interface PrivyLoginButtonProps {
  /** When true, renders a compact icon-only button suitable for tight mobile headers */
  compact?: boolean;
}

export function PrivyLoginButton({ compact = false }: PrivyLoginButtonProps) {
    ...
    if (compact) {
      return (
        <div
          ...
          className="w-9 h-9 rounded-full bg-[#83eef018] border border-[#83eef030] animate-pulse"
        />
      );
    }
    return (
    ...
    if (compact) {
      return (
        <button
          ...
          data-testid="button-sign-out-compact"
          aria-label={`Signed in as ${displayName} — tap to sign out`}
          className="w-9 h-9 rounded-full bg-[linear-gradient(135deg,rgba(131,238,240,0.25)_0%,rgba(63,176,179,0.15)_100%)] border border-[#83eef040] flex items-center justify-center flex-shrink-0 active:opacity-70 transition-opacity"
        >
          <span className="[font-family:'Inter',Helvetica] font-semibold text-[#83eef0] text-xs leading-none">
            {initial}
          </span>
        </button>
      );
  ...
  if (compact) {
    return (
      <button
        ...
        aria-label="Log in"
        className="w-9 h-9 rounded-full bg-[linear-gradient(135deg,rgba(131,238,240,0.9)_0%,rgba(63,176,179,0.9)_100%)] border-none shadow-[0_2px_12px_rgba(131,238,240,0.3)] flex items-center justify-center flex-shrink-0 active:opacity-80 transition-opacity"
      >
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
